...
In this paper, we focus on a different line of research on logic and social choice theory that uses interactive theorem provers to verify existing results in social choice theory.   The first results here used the Isabelle/HOL  \cite{Nipkow2009} and Mizar \cite{Wiedijk2007} interactive theorem provers to formalize different proofs  Arrow's impossibility theorem \cite{Geanakoplos2005}.  More recently, \cite{Eberl,Brandt2020} used Isabelle to verify the main theorem from \cite{Brandl2018}.  These projects demonstrate, as Nipkow \cite{Nipkow2009} notes,  that ``social choice theory turns out to be perfectly suitable for mechanical theorem proving."  In this paper, we provide further evidence of this by using the Lean interactive theorem prover to verify properties of a specific voting method (e.g., Condorcet consistency, independence of clones, etc.).

 
 The Lean proof assistant  \cite{Lean} is a framework that supports both interactive and automated theorem proving.   Lean's kernel is based on dependent type theory and   implements a version of the calculus of inductive constructions \cite{Coquand1988}  and Martin-L\"of type theory \cite{MartinLof1984}. There is an extensive and actively maintained  library of mathematical results formalized in Lean (see \url{https://leanprover-community.github.io/mathlib_docs/}).   
 
  
 
 In this paper, we develop  a framework for formalizing voting theory in  Lean.  In order to formalize voting theoretic axioms concerning adding or removing candidates and voters, we work in a variable-election setting whose formalization makes use of dependent types.   We illustrate the usefulness  of our  framework by verifying all the results about a recently proposed voting method Split Cycle from \cite{HP2020}.  
...
\section{Theorems}\label{Theorem}
