...
%\end{itemize}
\begin{itemize}
\item[] \texttt{def Prof : Type $\to$ Type $\to$ Type := }\\\texttt{$\lambda$ (V X : Type), V $\to$ X $\to$ X $\to$ Prop}
\end{itemize}
...
\begin{itemize}
\item[] \texttt{def condorcet\_winner [fintype V] (P : Prof V X) (x : X) :} \\ \texttt{Prop := $\forall y \neq x$, margin\_pos P x y}
\end{itemize}
