...
\begin{itemize}
\item \textit{universal domain} (resp.~finite universal domain): all profiles (resp.~finite profiles) are in the domain of the SCC.\\
\end{itemize}
...
We stress that the axiom definitions we formalized are the standard ones used throughout the literature, not ones tailored to any particular voting method, so others can immediately use our formalizations to verify results for any SCCs.
...
It would also be desirable to abstract away from the definition of margin in Definition \ref{MarginDef} to define voting methods and prove theorems in terms of an abstract quaternary relation $(a,b)\succ_\mathbf{P} (c,d)$ expressing that the strength of majority preference for $a$ over $b$ in profile $\mathbf{P}$ is stronger than the strength of majority preference for $c$ over $d$ in $\mathbf{P}$. One definition of $(a,b)\succ_\mathbf{P} (c,d)$ is that $Margin_\mathbf{P}(a,b)>Margin_\mathbf{P}(c,d)$. But if voter preference relations are non-linear, there are alternative, inequivalent definitions of $(a,b)\succ_\mathbf{P} (c,d)$, such as the \textit{winning votes} definition: $|\{i\in V\mid a\mathbf{P}_ib\}| \geq |\{i\in V\mid c\mathbf{P}_id\}|$. Schulze \cite{Schulze2011} considers other definitions besides margins and winning votes, and rather than settling for any one of them, lays down general axioms on the relation of relative strength of majority preference needed for proofs to go through. This is a natural next step in our project, which formalization facilitates: identify which of the properties of the $\succ_\mathbf{P}$ relation are actually used in our formalized proofs and then assume only those properties, rather than the definition of $\succ_\mathbf{P}$ by margins.%\footnote{Another benefit of thinking in terms of the relation $\succ_\mathbf{P}$ is that we can more easily handle social choice with infinite electorates. As subtraction is not defined for infinite cardinals, we cannot calculate margins by $|\{i\in V(\mathbf{P})\mid x\mathbf{P}_iy\}|-|\{i\in V(\mathbf{P})\mid y\mathbf{P}_ix\}|$ if the electorate is infinite.}
