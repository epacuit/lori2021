...
In this paper, we focus on a different line of research on logic and social choice theory that uses interactive theorem provers to verify existing results in social choice theory.   The first results here used the Isabelle/HOL  \cite{Nipkow2009} and Mizar \cite{Wiedijk2007} interactive theorem provers to formalize different proofs  Arrow's impossibility theorem \cite{Geanakoplos2005}.  More recently, \cite{Eberl,Brandt2020} used Isabelle to verify the main theorem from \cite{Brandl2018}.  These projects demonstrate, as Nipkow \cite{Nipkow2009} notes,  that ``social choice theory turns out to be perfectly suitable for mechanical theorem proving."  In this paper, we provide further evidence of this by developing a framework for formalizing voting theory using an interactive theorem prover. 

The most obvious benefit of using an interactive theorem prover same is that by a small trusted kernel.  
 Somewhere says what the benefits are: epistemic benefit of verification plus benefit of tracking exactly which assumptions (e.g., about voter preferences, cardinalities, choice of primitive concepts, etc.) are needed for which results.... 
