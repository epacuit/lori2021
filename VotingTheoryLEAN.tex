...
\begin{itemize}
\item[] \texttt{\textcolor{blue}{def} \textcolor{orange}{condorcet\_SCC} : SCC V X := $\lambda$ P, }
...
\begin{itemize}
\item[] \texttt{\textcolor{blue}{def} \textcolor{orange}{cycle} $\{$X: \textcolor{blue}{Type}$\}$ := $\lambda$ (P : X $\to$ X $\to$ Prop) (c : list X), \\
$\exists$ (e : c $\neq$ list.nil), list.chain P (c.last e) c}
\item[]
\item[] \texttt{\textcolor{blue}{def} \textcolor{orange}{split\_cycle\_defeat} : VCCR :=} 
    \texttt{$\lambda$ V X (P : Profile V X) (x y : X),}
    \texttt{$\forall$ [f: fintype V], 0 $<$ @margin V X f P x y $\wedge$}
